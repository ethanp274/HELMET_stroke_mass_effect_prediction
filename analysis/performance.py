...
def pull_HELMET_output(lookahead_hours):
    ...
    cv_mgb_output = pd.DataFrame(columns = ['cv', 'mls_categories', 'targets', 'predictions', 'proba_0', 'proba_1', 'proba_2', 'proba_3'])
    ...
    for i, df in enumerate([cv0_mgb, cv1_mgb, cv2_mgb, cv3_mgb, cv4_mgb]):
        mgb_output = pd.DataFrame({
            ...
            'proba_0': df['0_prediction_probability'], 
            'proba_1': df['0_3_prediction_probability'], 
            'proba_2': df['3_8_prediction_probability'], 
            'proba_3': df['8_up_prediction_probability']
        })
        mgb_output['cv'] = i
    ...
    return cv_mgb_output, cv_mgb_filt_output, cv_bmc_output, cv_bmc_filt_output
...
def create_edema_baseline(OLTV_df, OTV_df, test_ptids, scaler_list, lookahead_hours, hosp):
    ...
    cv_output = pd.DataFrame(columns = ['cv', 'mls_categories', 'targets', 'predictions', 'proba_0', 'proba_1', 'proba_2', 'proba_3'])
...
def generate_EDEMA_output(lookahead_hours):
    ...
    return mgb_output, mgb_filt_output, bmc_output, bmc_filt_output

#_______________________________________________________________________________________________________________________

def calc_metrics(cv_mgb_output, cv_mgb_filt_output, cv_bmc_output, cv_bmc_filt_output, lookahead_hours):
    ...
    # Calculate per-split metrics
    cv_mgb_results_AGNI = pd.DataFrame(columns = ['accuracy', 'sensitivity', 'specificity'])
    cv_mgb_filt_results_AGNI = pd.DataFrame(columns = ['accuracy', 'sensitivity', 'specificity'])
    cv_bmc_results_AGNI =  pd.DataFrame(columns = ['accuracy', 'sensitivity', 'specificity'])
    cv_bmc_filt_results_AGNI = pd.DataFrame(columns = ['accuracy', 'sensitivity', 'specificity'])

    cv_mgb_results_WORSEN = pd.DataFrame(columns = ['accuracy', 'sensitivity', 'specificity'])
    cv_mgb_filt_results_WORSEN = pd.DataFrame(columns = ['accuracy', 'sensitivity', 'specificity'])
    cv_bmc_results_WORSEN = pd.DataFrame(columns = ['accuracy', 'sensitivity', 'specificity'])
    cv_bmc_filt_results_WORSEN = pd.DataFrame(columns = ['accuracy', 'sensitivity', 'specificity'])

    for cv in range(5):
        mgb_output_n = cv_mgb_output[cv_mgb_output['cv'] == cv].reset_index(drop = True)
        bmc_output_n = cv_bmc_output[cv_bmc_output['cv'] == cv].reset_index(drop = True)

        mgb_results_AGNI, mgb_filt_results_AGNI = get_acc_sens_spec_AGNI(mgb_output_n)
        bmc_results_AGNI, bmc_filt_results_AGNI = get_acc_sens_spec_AGNI(bmc_output_n)

        mgb_results_WORSEN, mgb_filt_results_WORSEN = get_acc_sens_spec_WORSEN(mgb_output_n)
        bmc_results_WORSEN, bmc_filt_results_WORSEN = get_acc_sens_spec_WORSEN(bmc_output_n)

        cv_mgb_results_AGNI.loc[cv] = mgb_results_AGNI
        cv_mgb_filt_results_AGNI.loc[cv] = mgb_filt_results_AGNI
        cv_bmc_results_AGNI.loc[cv] = bmc_results_AGNI
        cv_bmc_filt_results_AGNI.loc[cv] = bmc_filt_results_AGNI

        cv_mgb_results_WORSEN.loc[cv] = mgb_results_WORSEN
        cv_mgb_filt_results_WORSEN.loc[cv] = mgb_filt_results_WORSEN
        cv_bmc_results_WORSEN.loc[cv] = bmc_results_WORSEN
        cv_bmc_filt_results_WORSEN.loc[cv] = bmc_filt_results_WORSEN
    ...
    avg_mgb_results_AGNI = process_average_metrics(cv_mgb_results_AGNI)
    avg_mgb_filt_results_AGNI = process_average_metrics(cv_mgb_filt_results_AGNI)
    avg_bmc_results_AGNI = process_average_metrics(cv_bmc_results_AGNI)
    avg_bmc_filt_results_AGNI = process_average_metrics(cv_bmc_filt_results_AGNI)

    avg_mgb_results_WORSEN = process_average_metrics(cv_mgb_results_WORSEN)
    avg_mgb_filt_results_WORSEN = process_average_metrics(cv_mgb_filt_results_WORSEN)
    avg_bmc_results_WORSEN = process_average_metrics(cv_bmc_results_WORSEN)
    avg_bmc_filt_results_WORSEN = process_average_metrics(cv_bmc_filt_results_WORSEN)

    # Print results
    print(f"Results for MGB: ")
    print_AUCs(mgb_AUROC, mgb_filt_AUROC, mgb_AUPRC, mgb_filt_AUPRC)
    print_results(avg_mgb_results_WORSEN, avg_mgb_filt_results_WORSEN)

    print(f"Results for BMC: ")
    print_AUCs(bmc_AUROC, bmc_filt_AUROC, bmc_AUPRC, bmc_filt_AUPRC)
    print_results(avg_bmc_results_WORSEN, avg_bmc_filt_results_WORSEN)

    return mgb_ROC_data, mgb_ROC_data_filt, mgb_PRC_data, mgb_PRC_data_filt, bmc_ROC_data, bmc_ROC_data_filt, bmc_PRC_data, bmc_PRC_data_filt
...
def create_plots(xgb_overall, xgb_filt, edema_overall, edema_filt, type, hosp, hours):
    ...
    plt.savefig(f'results/figures/shap/{hosp}_{hours}hr_{type}_plot.png', dpi=450)
...
if __name__ == "__main__":
    ...
    # Get data
    HELMET_mgb_output_8, HELMET_mgb_filt_output_8, HELMET_bmc_output_8, HELMET_bmc_filt_output_8 = pull_HELMET_output(8)
    HELMET_mgb_output_24, HELMET_mgb_filt_output_24, HELMET_bmc_output_24, HELMET_bmc_filt_output_24 = pull_HELMET_output(24)

    EDEMA_mgb_output_8, EDEMA_mgb_filt_output_8, EDEMA_bmc_output_8, EDEMA_bmc_filt_output_8 = generate_EDEMA_output(8)
    EDEMA_mgb_output_24, EDEMA_mgb_filt_output_24, EDEMA_bmc_output_24, EDEMA_bmc_filt_output_24 = generate_EDEMA_output(24)

    # Calculate metrics, print metrics, and get plot data

    print('HELMET-8')
    HELMET_mgb_8_roc, HELMET_mgb_8_roc_filt, HELMET_mgb_8_prc, HELMET_mgb_8_prc_filt, HELMET_bmc_8_roc, HELMET_bmc_8_roc_filt, HELMET_bmc_8_prc, HELMET_bmc_8_prc_filt = calc_metrics(HELMET_mgb_output_8, HELMET_mgb_filt_output_8, HELMET_bmc_output_8, HELMET_bmc_filt_output_8, 8)
    
    print('HELMET-24')
    HELMET_mgb_24_roc, HELMET_mgb_24_roc_filt, HELMET_mgb_24_prc, HELMET_mgb_24_prc_filt, HELMET_bmc_24_roc, HELMET_bmc_24_roc_filt, HELMET_bmc_24_prc, HELMET_bmc_24_prc_filt = calc_metrics(HELMET_mgb_output_24, HELMET_mgb_filt_output_24, HELMET_bmc_output_24, HELMET_bmc_filt_output_24, 24)

    print('EDEMA-8')
    EDEMA_mgb_8_roc, EDEMA_mgb_8_roc_filt, EDEMA_mgb_8_prc, EDEMA_mgb_8_prc_filt, EDEMA_bmc_8_roc, EDEMA_bmc_8_roc_filt, EDEMA_bmc_8_prc, EDEMA_bmc_8_prc_filt = calc_metrics(EDEMA_mgb_output_8, EDEMA_mgb_filt_output_8, EDEMA_bmc_output_8, EDEMA_bmc_filt_output_8, 8)
    
    print('EDEMA-24')
    EDEMA_mgb_24_roc, EDEMA_mgb_24_roc_filt, EDEMA_mgb_24_prc, EDEMA_mgb_24_prc_filt, EDEMA_bmc_24_roc, EDEMA_bmc_24_roc_filt, EDEMA_bmc_24_prc, EDEMA_bmc_24_prc_filt = calc_metrics(EDEMA_mgb_output_24, EDEMA_mgb_filt_output_24, EDEMA_bmc_output_24, EDEMA_bmc_filt_output_24, 24)

    # Create plots
    create_plots(HELMET_mgb_8_roc, HELMET_mgb_8_roc_filt, EDEMA_mgb_8_roc, EDEMA_mgb_8_roc_filt, 'roc', 'MGB', 8)
    create_plots(HELMET_bmc_8_roc, HELMET_bmc_8_roc_filt, EDEMA_bmc_8_roc, EDEMA_bmc_8_roc_filt, 'roc', 'BMC', 8)
    create_plots(HELMET_mgb_24_roc, HELMET_mgb_24_roc_filt, EDEMA_mgb_24_roc, EDEMA_mgb_24_roc_filt, 'roc', 'MGB', 24)
    create_plots(HELMET_bmc_24_roc, HELMET_bmc_24_roc_filt, EDEMA_bmc_24_roc, EDEMA_bmc_24_roc_filt, 'roc', 'BMC', 24)

    create_plots(HELMET_mgb_8_prc, HELMET_mgb_8_prc_filt, EDEMA_mgb_8_prc, EDEMA_mgb_8_prc_filt, 'prc', 'MGB', 8)
    create_plots(HELMET_bmc_8_prc, HELMET_bmc_8_prc_filt, EDEMA_bmc_8_prc, EDEMA_bmc_8_prc_filt, 'prc', 'BMC', 8)
    create_plots(HELMET_mgb_24_prc, HELMET_mgb_24_prc_filt, EDEMA_mgb_24_prc, EDEMA_mgb_24_prc_filt, 'prc', 'MGB', 24)
    create_plots(HELMET_bmc_24_prc, HELMET_bmc_24_prc_filt, EDEMA_bmc_24_prc, EDEMA_bmc_24_prc_filt, 'prc', 'BMC', 24)
